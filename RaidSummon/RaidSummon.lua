--can you check if someone is in combat if not in same zone as you before starting the cast ?
--sorts warlocks firsts then healers/mage prio ?
...
--asks if player and taret are in combat. Is that possible ?	
		if UnitID then
...
--Summoning Speech				
					if GetSubZoneText() == "" then
						SendChatMessage("Summoning ".. name .. " to "..GetZoneText(), "RAID")
						SendChatMessage("Summoning you to "..GetZoneText(), "WHISPER", nil, name)
					else
						SendChatMessage("Summoning ".. name .. " to "..GetZoneText() .. " - " .. GetSubZoneText(), "RAID")
				...
				elseif not RaidSummonOptions.zone and RaidSummonOptions.whisper then
					SendChatMessage("Summoning ".. name, "RAID")
				...
				DEFAULT_CHAT_FRAME:AddMessage("Summoned Player is in combat")
			end
		else
			DEFAULT_CHAT_FRAME:AddMessage("Summoned Player " .. tostring(name) .. " not found in raid. UnitID: " .. tostring(UnitID))
		end

--Rightclick = remove from list		
